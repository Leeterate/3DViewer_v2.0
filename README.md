...
В данном проекте требовалось реализовать на языке С++ усовершенствованную версию программы для просмотра 3D моделей в каркасном виде (3D Viewer). В процессе разработки помимо паттерна MVC требовалось использовать как минимум три различных паттерна проектирования.

Сами модели предполагалось загружать из файлов формата .obj, требовалась возможность просматривать их на экране с возможностью вращения, масштабирования и перемещения.

В данном проекте было достаточно реализовать поддержку списка вершин и поверхностей. Все остальное - не являлось обязательным.
...
- В программе должен быть реализован графический пользовательский интерфейс, на базе любой GUI-библиотеки (в данном QT)
...
Copyright © 2023 Wartzkay, Waresher, Dimpleka.
